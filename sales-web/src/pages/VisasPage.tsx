  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map((v) => (
          <article key={v.id} className="card-interactive group flex cursor-pointer flex-col overflow-hidden" onClick={() => setSelected(v)}>
            ...
            <div className="mt-4 flex items-end justify-between border-t border-slate-100 pt-3">
              ...
              <button className="btn-deal py-1.5 text-sm">立即办理</button>
            </div>
            </div>
          </article>
        ))}
      ...
      {filtered.length === 0 && (
        <div className="card text-ink-soft">没有匹配的签证产品。</div>
      )}
