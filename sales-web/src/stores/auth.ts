...
const AUTH_STORAGE_KEY = 'ftm-auth';
 ...
 * 所有并发 refreshSession（定时续期 / 多个 401 重试 / 多标签 / 重复挂载）共享同一个 promise。
...
interface AuthState {
  ...
  logout: () => Promise<void>;
  /**
   * 用 refreshToken 换新 accessToken，保持会话不掉线。
...
async function runRefresh(staleAccessToken: string | null): Promise<RefreshOutcome> {
  // 取内存 tokens 之前，先从 localStorage 同步一次：后台隐藏标签的轮询用的是内存里几分钟前的
  // 旧 refreshToken，而兄弟标签可能早已把它轮换掉。若不同步就直接拿旧 token 去刷 → 撞后端
  // 「很久以前作废」的重放判定 → 撤销该用户全部会话。rehydrate 先把新令牌读进来。
  ...
  // 本就没到期）→ 无需再刷，直接放行。必须排除 staleAccessToken 本身：服务端已经拒过它，
  // 本地 exp 说它新鲜也不足为凭。
  ...
  try {
    ...
    // 网络抖动 / 5xx / 竞争未收敛：保留会话（绝不因一次瞬时故障把正在下单的用户踢下线），
...
// refreshed → 带新 token 重试原请求；expired → 放行原始 401 走登出（只读请求还会退回游客视角
// 再试一次）；unavailable → apiFetch 抛 AUTH_REFRESH_UNAVAILABLE（非 401），会话原地保留。
registerAuthRefresh((staleAccessToken) => useAuth.getState().refreshSession(staleAccessToken));

// ── 跨标签页实时同步（幂等，只在模块首次加载时注册一次）───────────────────────
// 兄弟标签轮换出新令牌、或登出时，会写入 localStorage 的 AUTH_STORAGE_KEY —— 触发本页
// storage 事件，立刻 rehydrate 跟上，避免本页继续拿旧令牌请求。
// · storage 事件只由「其它」标签页的写入触发，本页自己 set 不会触发它。
