...
import {
  ...
  settlementDiscountKindSchema,
  settlementTierSchema,
} from './enums.js';
import { routeKeySchema } from './settlement-rates.js';

const dateStr = z.string().regex(/^\d{4}-\d{2}-\d{2}$/u, '日期格式应为 YYYY-MM-DD');
const tierSchema = settlementTierSchema;
const kindSchema = settlementDiscountKindSchema;
