...
> **现状判定**（Codex GPT-5 2026-04-16 审查）：
> **是**：可演示的直销/分销样机
> **不是**：可卖 license 的旅行平台内核
>
> 50+ 家旅行社同时使用前，以下 5 类功能和 6 处架构债必须重做。
...
| 真实承接 1 家客户业务 | 🟡 需补履约 + 财务 | M6 上线前完成 |
| 支撑 10+ 家并发使用 | 🔴 需多租户重构 | V2 目标 |
| 支撑 50+ 家 | 🔴 需完整重构 | V3 目标 |
...
### 1. 多租户 / SaaS 内核
**现状**：`Agent` 是分销树不是 licensee 租户。50 家旅行社会互相串配置。

**缺什么**：
- `Tenant`（licensee 租户）
- `AgencyLicense`（订阅套餐 标准/专业/旗舰 + 到期日 + 用量配额）
- `Brand / Domain`（每家自己的品牌 logo、域名、配色）
- `FeatureFlag`（不同套餐开不同功能）
- **隔离**：数据按 tenantId 强制隔离，所有 Prisma 查询都要加 tenantId filter

**判断标准**：卖给第 4 家旅行社时，不能让他们的代理 / 订单 / 定价规则和前 3 家混在一起。
...
```
权限从纯 RBAC 升级到 "角色 + 作用域"（e.g. 客服角色 + scope=某区域）。
...
| **M6** | 1–2 个月 | 多租户 + 财务结算 + 履约内核 → 可卖第 1 家真客户 |
| **M7** | 2–3 个月 | 集成 + 部署 + 合规 → AWS 上线 |
| **M8** | 3 个月 | 真微信支付 + OCR + Prophet ML |
| **V2** | 6 个月 | 支撑 10+ 租户 |
| **V3** | 12 个月 | 50+ 租户 + 多币种 + 国际市场 |
