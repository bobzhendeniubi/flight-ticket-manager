...
| **审计日志** | ✅ **真后端** | `AuditLog` 表 + `lib/audit.ts` 助手 · `/audit-logs` 查询 API · orders/settlements/customers/travelers/fulfillment 操作自动写日志（actor/target/IP/before/after/severity）· admin AuditLogsPage 接通 |
