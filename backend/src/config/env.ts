...
const EnvSchema = z.object({
  ...
  // ═══════════════════════════════════════════════════════════
  // 航班库存：no-show 回程恢复的超售上限
  // 场景：航司发来 no-show 名单，票务把没来客人的回程座位释放回库存重卖；之后代理来说
  // 「这位客人还要回程」，票务恢复回程到原班次。此时原班次可能已被卖掉，业务口径是
  // **允许超售**（航司包机位本来就按 no-show 率放量），前端二次确认 + 后端 CRITICAL 审计。
  // 本上限只防手滑（如把 1 人恢复成 100 人），不是业务闸。
  // ⚠ 与 FLIGHT_MAX_OVERSELL_SEATS 语义不同：那条管的是「班次容量下调到低于已售」的写入守卫，
  // 两者互不复用，各自独立取值。
  // ═══════════════════════════════════════════════════════════
