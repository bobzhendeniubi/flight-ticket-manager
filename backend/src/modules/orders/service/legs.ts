...
export async function _assessCancelLeg(svc: OrderService, db: Prisma.TransactionClient, orderId: string, leg: FlightLegSide): Promise<{
...
/** 航段行 → 预检/审计用的可读快照。 */
export function _describeLeg(svc: OrderService, item: CancelLegItemSnapshot): CancelLegItemView {
...
export async function previewCancelLeg(
  ...
  const { order, legItem, blockers, warnings, ackWarnings } = await svc._assessCancelLeg(
    prisma,
    orderId,
    leg,
  );
  ...
  const policyFee = legItem
    ? await svc._quoteLegCancelFee(prisma, legItem.id, legAmountCny, new Date())
    : null;
  ...
  return {
    ...
    returnItem: legItem ? svc._describeLeg(legItem) : null,
    policyFee,
    netReductionCny,
    maxRefundCny,
    currentTotalCny,
...
export async function previewCancelReturnLeg(svc: OrderService, orderId: string, actor: { userId: string; role: UserRole }): Promise<CancelLegPreview> {
...
export async function cancelLeg(
  ...
  const audit = await runOrderMutation<CancelLegAudit>({
    ...
    const now = new Date();
    const policyFee = await svc._quoteLegCancelFee(tx, legItem.id, legAmountCny, now);
...
export async function _assessNoShow(
  ...
  if (picked && picked.length > 0) {
    ...
    if (unknown.length > 0) {
      ...
      const splitAssessment = await svc.assessOrderSplitForNoShow(db, orderId, picked);
 ...
 */
export async function assessOrderSplitForNoShow(svc: OrderService, db: Prisma.TransactionClient, orderId: string, passengerIds: string[]): Promise<string[]> {
  ...
  const assessment = await svc.assessOrderSplit(db, source, passengerIds, {
...
export function _describeNoShowLeg(svc: OrderService, item: CancelLegItemSnapshot): NoShowLegView {
  const sched = item.flightSchedule;
  return {
    orderItemId: item.id,
    description: item.description,
...
export async function previewNoShow(
  ...
  const assessed = await svc._assessNoShow(
    prisma,
    orderId,
    body.passengerIds,
  ...
  return {
    ...
    outboundItem: assessed.outboundItem
      ? svc._describeNoShowLeg(assessed.outboundItem)
      : null,
    returnItem: assessed.returnItem
      ? {
          ...svc._describeNoShowLeg(assessed.returnItem),
...
export async function markNoShow(
  ...
  if (!needsSplit) {
    const audit = await svc._executeNoShow(orderId, input, actor, null);
  ...
  const preSplitAssessed = await svc._assessNoShow(
    prisma,
    orderId,
    undefined,
    input.releaseReturn,
  ...
  try {
    audit = await svc._executeNoShow(
      split.targetOrderId,
      input,
      actor,
...
export async function _assessRestoreReturnLeg(svc: OrderService, db: Prisma.TransactionClient, orderId: string): Promise<{
    order: CancelLegOrderSnapshot;
...
export async function previewRestoreReturnLeg(svc: OrderService, orderId: string, actor: { userId: string; role: UserRole }): Promise<RestoreReturnLegPreview> {
  if (!actorCan(actor, 'orders.cancel_leg')) {
    throw new ForbiddenError('仅运营/管理员可恢复回程');
  }
  const assessed = await svc._assessRestoreReturnLeg(prisma, orderId);
...
export async function previewVoidReturnLeg(svc: OrderService, orderId: string, actor: { userId: string; role: UserRole }): Promise<VoidReturnLegPreview> {
  if (!actorCan(actor, 'orders.cancel_leg')) {
    throw new ForbiddenError('仅运营/管理员可作废回程');
  }
  const assessed = await svc._assessVoidReturnLeg(prisma, orderId);
  return {
    eligible: assessed.blockers.length === 0,
    blockers: assessed.blockers,
    departed: assessed.departed,
...
export async function _assessVoidReturnLeg(svc: OrderService, db: Prisma.TransactionClient, orderId: string): Promise<{
    order: CancelLegOrderSnapshot;
...
export const LEG_ZH: Record<FlightLegSide, string> = { OUTBOUND: '去程', RETURN: '回程' };

/**
 * 被取消的航段行在描述前打的留痕前缀（幂等：已带前缀不再叠加）。
 * 字面量本体在 orders.leg-status.ts（对外脱敏要剥掉它们，两处各写一份必然漂移）。
 */
export const LEG_CANCELLED_PREFIX: Record<FlightLegSide, string> = {
...
export const LEG_CANCEL_FEE_REASON: Record<FlightLegSide, PriceAdjustmentReasonDisplay> = {
  OUTBOUND: 'OUTBOUND_LEG_CANCEL_FEE',
  RETURN: 'RETURN_LEG_CANCEL_FEE',
};

/** 取消航段准入评估要读的订单快照（select 与类型同源，改一处即改两处）。 */
export async function loadOrderForLegCancel(db: Prisma.TransactionClient, orderId: string) {
...
export type CancelLegOrderSnapshot = NonNullable<Awaited<ReturnType<typeof loadOrderForLegCancel>>>;
export type CancelLegItemSnapshot = CancelLegOrderSnapshot['items'][number];
...
export function readSnapshotDate(raw: unknown): Date | null {
 ...
 */
export function resolveReturnReleaseState<
...
export function stripNoShowPrefix(description: string): string {
...
export function stripReturnReleasedPrefix(description: string): string {
...
export function buildTicketWorkOrderTitle(
  action: string,
  orderNumber: string,
  legZh: string,
 ...
 */
export async function notifyWorkOrderCreatedToWecom(orderNumber: string, title: string | null): Promise<void> {
  if (!title) return;
