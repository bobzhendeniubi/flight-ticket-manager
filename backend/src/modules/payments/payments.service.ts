...
export class PaymentsService {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const payload =
        ...
        !Array.isArray(payment.gatewayPayload)
          ...
          : null;
      if (payload?.swapTransfer === true) {
        throw new BadRequestError(
          '这笔收款是换人转出的转入款，不能单独撤销；如需回退请到源订单侧按换人转出的反向流程处理。',
        );
      }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const sourcePayload =
        ...
        !Array.isArray(lockedPayment.gatewayPayload)
          ...
          : {};
      if (sourcePayload.swapTransfer === true) {
        throw new BadRequestError(
          '这笔收款是换人转出的转入款，不能再次整笔转移；如需回退请到源订单侧按换人转出的反向流程处理。',
        );
      }
  ...
  /**
   * 换人转出：源单只保留运营手填的换人费，其余净收款转入另一张新订单。
   *
   * 源单的 Payment 行不搬也不改状态——它们证明钱曾经收在源单上；资金流出由
   * 一条非现金 Refund 表达，避免把「转入新单」误当成需要再次打款的现金退款。
   */
  async swapTransfer(
    orderId: string,
    input: { targetOrderNumber: string; transferFeeCny: number; reason: string },
    actor: AuditActor,
  ): Promise<{
    sourceOrder: {
      id: string;
    ...
    targetOrder: { id: string; orderNumber: string; paidAmount: number; status: OrderStatus };
    transferFeeCny: number;
    transferredAmount: number;
    refundId: string;
    newPaymentId: string;
  }> {
    // 这是运营侧的跨订单资金操作；把权限闸放在事务前，避免非运营请求触碰任何账目查询。
    ...
    if (
      ...
      throw new ForbiddenError('仅运营/管理员可做换人转出');
    }
    const statusActor = { userId: actorUserId, role: actorRole };

    const targetOrderNumber = input.targetOrderNumber.trim();
    const reason = input.reason.trim();
    const transferFeeCny = input.transferFeeCny;
    if (!Number.isInteger(transferFeeCny) || transferFeeCny < 0) {
      throw new BadRequestError('换人费必须是大于等于 0 的整数');
    }
    if (!targetOrderNumber) throw new BadRequestError('请填写转入的订单号');
    if (!reason) throw new BadRequestError('请填写换人转出原因');
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!target || target.deletedAt) {
        throw new BadRequestError(
          '转入订单不存在或已在回收站，不能做换人转出。请核对订单号并先恢复订单。',
        );
      }
      if (target.id === orderId) throw new BadRequestError('不能转到同一张订单');
      ...
      const lockedOrders = new Map<string, LockedOrder>();
      // 两单统一按 id 排序加锁；A→B 与 B→A 并发时必须使用同一锁序，避免死锁。
      for (const id of [orderId, target.id].sort()) {
        const rows = await tx.$queryRaw<LockedOrder[]>`
          SELECT id, "orderNumber", total, "adjustmentCny", "paidAmount", "prepaymentOffset",
                 status, "deletedAt", "paymentsLocked", "agentId", "internalNotes"
          FROM "Order"
          WHERE id = ${id}
          FOR UPDATE
        `;
      ...
      const sourceOrder = lockedOrders.get(orderId);
      const targetOrder = lockedOrders.get(target.id);
      if (!sourceOrder || !targetOrder) throw new NotFoundError('订单不存在');
      ...
      if (sourceOrder.deletedAt) {
        throw new BadRequestError('订单在回收站（已软删），不可换人转出；如需操作请先恢复');
      }
      if (!SEAT_HOLDING_STATUSES.includes(sourceOrder.status)) {
        throw new BadRequestError(
          `订单 ${sourceOrder.orderNumber} 当前状态为「${sourceOrder.status}」，仅占座中的有效订单可换人转出。`,
        );
      }
      if (!CANCELLABLE_STATUSES.has(sourceOrder.status)) {
        throw new BadRequestError(
          `订单 ${sourceOrder.orderNumber} 当前状态不可走换人转出退款通道，请按退款流程处理。`,
        );
      }
      if (sourceOrder.paymentsLocked) {
        throw new ConflictError(
          '订单收款已锁定（财务复核完成），请先在订单收款区解锁再做换人转出',
        );
      }
      ...
      if (pendingRefundCount > 0) {
        throw new ConflictError('该订单已有待处理的退款申请，请先处理该单待办的退款申请再做换人转出');
      }
      ...
      if (blockedTargetStatuses.includes(targetOrder.status)) {
        throw new BadRequestError(
          '不能把收款转移到已取消/已退款/已失效的订单。请先恢复目标订单到可收款状态后再做换人转出。',
        );
      }
      assertOrderAcceptsFunds(targetOrder);

      const refundedTotal = await sumCompletedRefundsWithinTx(tx, sourceOrder.id);
      const targetRefundedTotal = await sumCompletedRefundsWithinTx(tx, targetOrder.id);
      const paidAmountBefore = round2(Number(sourceOrder.paidAmount));
      const netPaid = round2(paidAmountBefore - refundedTotal);
      const targetPaidBefore = round2(Number(targetOrder.paidAmount));
      const targetNetBefore = round2(targetPaidBefore - targetRefundedTotal);
      if (toCents(netPaid) <= 0) {
        throw new BadRequestError('该订单没有可转出的已收款');
      }
      if (toCents(transferFeeCny) > toCents(netPaid)) {
        throw new BadRequestError(
          `换人费 ¥${transferFeeCny.toFixed(2)} 不能超过该单净收款 ¥${netPaid.toFixed(2)}`,
        );
      }

      const transferredAmount = round2(netPaid - transferFeeCny);
      if (toCents(transferredAmount) <= 0) {
        throw new BadRequestError(
          '换人费已等于/超过该单净收款，没有可转出的余额；如只是要关单请走退款流程',
        );
      }

      // OFFSET 有独立余额账本；若允许这里再转一次，REFUNDED 分支还会回补余额，形成双花。
      const offset = await tx.prepaymentTransaction.findFirst({
        where: { orderId: sourceOrder.id, type: PrepaymentTxType.OFFSET },
        select: { id: true },
      });
      if (offset) {
        throw new BadRequestError('该订单用过代理预存余额抵扣，请走退款流程处理，不能走换人转出');
      }

      const actionAt = new Date();
      const transferredAt = businessDateTime(actionAt);
      const refundReason =
        `换人转出至订单 ${targetOrder.orderNumber}（换人费 ¥${transferFeeCny}）：${reason}` +
        '。这笔钱已转入另一张订单，请勿再打款。';
      const refund = await tx.refund.create({
        data: {
          orderId: sourceOrder.id,
          amount: new Prisma.Decimal(transferredAmount),
      ...
      // 先申请退款再走状态机，才能满足 REFUND_REQUESTED 的账目闸；这一步也原子释放 A 的座位。
      await this.orderService._updateStatusWithinTx(
        tx,
        sourceOrder.id,
      ...
      const latestPayment = await tx.payment.findFirst({
        where: {
          orderId: sourceOrder.id,
          status: PaymentStatus.SUCCEEDED,
          method: { not: PaymentMethod.AGENT_PREPAYMENT },
        },
        orderBy: { createdAt: 'desc' },
        select: { method: true },
      });
      // 当前枚举没有独立的「线下/其他」值；人工线下收款沿用 BANK_CARD 记录，故无可复用方式时回退它。
      const paymentMethod = latestPayment?.method ?? PaymentMethod.BANK_CARD;
      const credited = await this._creditOrderPaymentWithinTx(
        tx,
        targetOrder.id,
        {
          ...
          overchargeHint: '请先调整转入订单的价格再做换人转出。',
        },
        statusActor,
        pendingFulfillmentTaskIds,
      );

      // A 的 Payment 行一律保留原状：钱曾经确实收在 A 上，流出由 Refund 表达，不能再拆 Payment 或凭证分配。
      await this.orderService._updateStatusWithinTx(
        tx,
        sourceOrder.id,
        OrderStatus.REFUNDED,
      ...
      const sourceNote =
        `【换人转出】${transferredAt} 已转出 ¥${transferredAmount} 至订单 ${targetOrder.orderNumber}，` +
        `本单留存换人费 ¥${transferFeeCny}。原因：${reason}`;
      const targetNote =
        `【换人转入】${transferredAt} 由订单 ${sourceOrder.orderNumber} 转入 ¥${transferredAmount}。` +
        `原因：${reason}`;
      const appendNote = (existing: string | null): string =>
        existing ? `${existing}\n${sourceNote}` : sourceNote;
      const appendTargetNote = (existing: string | null): string =>
        existing ? `${existing}\n${targetNote}` : targetNote;
      await tx.order.update({
        where: { id: sourceOrder.id },
        data: { internalNotes: appendNote(sourceOrder.internalNotes) },
      });
      await tx.order.update({
        where: { id: targetOrder.id },
        data: { internalNotes: appendTargetNote(targetOrder.internalNotes) },
      });

      const persistedRefund = await tx.refund.findUnique({
        where: { id: refund.id },
        select: { status: true },
      });
      if (persistedRefund?.status !== RefundStatus.COMPLETED) {
        throw new Error('换人转出退款未落到已完成状态，事务已回滚');
      }

      // 状态机和入账内核都已执行后仍重读账面真值；不能用上面内存里的推导值自证守恒。
      const persistedOrders = new Map<string, Prisma.Decimal>();
      for (const id of [sourceOrder.id, targetOrder.id].sort()) {
        const rows = await tx.$queryRaw<Array<{ id: string; paidAmount: Prisma.Decimal }>>`
        ...
        if (!persisted) throw new Error('换人转出订单重读失败，事务已回滚');
        persistedOrders.set(id, persisted.paidAmount);
      }
      const sourcePaidAmountAfter = round2(Number(persistedOrders.get(sourceOrder.id) ?? 0));
      const targetPaidAmountAfter = round2(Number(persistedOrders.get(targetOrder.id) ?? 0));
      const sourceRefundedTotalAfter = await sumCompletedRefundsWithinTx(tx, sourceOrder.id);
      const targetRefundedTotalAfter = await sumCompletedRefundsWithinTx(tx, targetOrder.id);
      const sourceNetAfter = round2(sourcePaidAmountAfter - sourceRefundedTotalAfter);
      const targetNetAfter = round2(targetPaidAmountAfter - targetRefundedTotalAfter);
      if (toCents(sourceNetAfter) !== toCents(transferFeeCny)) {
        throw new Error(
          `换人转出后源单净收款校验失败：实际 ¥${sourceNetAfter.toFixed(2)}，换人费 ¥${transferFeeCny.toFixed(2)}，事务已回滚`,
        );
      }
      if (
        toCents(netPaid + targetNetBefore) !==
        toCents(sourceNetAfter + targetNetAfter)
      ) {
        throw new Error('换人转出金额守恒校验失败，事务已回滚');
      }
      ...
      return {
        sourceOrder: {
          id: sourceOrder.id,
          orderNumber: sourceOrder.orderNumber,
          paidAmount: sourcePaidAmountAfter,
          netPaidAmount: sourceNetAfter,
          status: OrderStatus.REFUNDED,
        },
        targetOrder: {
          id: targetOrder.id,
          orderNumber: targetOrder.orderNumber,
          paidAmount: targetPaidAmountAfter,
        ...
        refundId: refund.id,
        newPaymentId: credited.paymentId,
        sourceStatusBefore: sourceOrder.status,
        sourceNetBefore: netPaid,
        reason,
      };
    });
    ...
    // 换人转出最终关闭 A；即使旧数据残留了占座兜底任务，提交后也要取消它。
    try {
      const { cancelSeatHoldRelease } = await import('../../queues/queue.js');
      await cancelSeatHoldRelease(result.sourceOrder.id);
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error('[payments] failed to cancel seat-hold job for', result.sourceOrder.id, err);
    }
    ...
    if (releasedSeatClassIds.length > 0) {
      try {
        ...
        // eslint-disable-next-line no-console
        console.error('[payments] failed to enqueue waitlist-check for', result.sourceOrder.id, err);
      }
    }
    ...
    void writeAudit({
      ...
      targetLabel: result.sourceOrder.orderNumber,
      before: {
        status: result.sourceStatusBefore,
        netPaidAmount: result.sourceNetBefore,
      },
      after: {
        targetOrderNumber: result.targetOrder.orderNumber,
        transferFeeCny: result.transferFeeCny,
