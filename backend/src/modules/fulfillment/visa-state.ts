...
/**
 * 按人推进送签进度（签证台按人/批量标记的核心）：逐人过守卫 → 通过者一次 updateMany。
 * 订单存活等**非签证**守卫由调用方先判（那是订单是否参与履约的口径，不属于状态机）。
 * 不重派生任务、不派生办结——那两步按受影响订单去重后由调用方接着做。
 */
export async function applyVisaProgress(
  db: VisaDb,
  input: {
    passengers: ReadonlyArray<{
      id: string;
      visaExempt: boolean | null | undefined;
      visaSubmissionStatus?: VisaSubmissionStatus | null;
      orderVisaStatus?: VisaRequirement | null;
    }>;
    to: VisaSubmissionStatus;
  },
): Promise<{ okIds: string[]; rejected: Array<{ id: string; reason: string }> }> {
  const okIds: string[] = [];
  const rejected: Array<{ id: string; reason: string }> = [];
  for (const p of input.passengers) {
    const t = transitionPassengerVisa(
      {
        orderVisaStatus: p.orderVisaStatus ?? null,
        visaExempt: p.visaExempt,
        visaSubmissionStatus: p.visaSubmissionStatus,
        // 单人视角用不到 allPassengersExempt（只影响 HAS_VISA 单里自备签乘客的派生态，
        // 而自备签乘客在这里一律被守卫拒掉）
        allPassengersExempt: false,
      },
      visaProgressEvent(input.to, p.visaSubmissionStatus),
    );
    if (t.ok) okIds.push(p.id);
    else rejected.push({ id: p.id, reason: t.reason });
  }
  if (okIds.length > 0) await writePassengerVisaProgress(db, { passengerIds: okIds }, input.to);
  return { okIds, rejected };
}
