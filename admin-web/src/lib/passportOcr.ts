...
// ── 护照图压缩（避免后端 413）────────────────────────────────────────────────
// 多人团（如 9 人各一张护照图）会把请求体撑爆后端上限。OCR 录单也要把存库图缩小：
// 长边 ≤1600px、转 JPEG、目标 ≤~700KB（data-URL）。OCR 识别本身仍用原始 File，不受影响。
...
/**
 * 把护照图片 File 压成存库用的 data-URL：canvas 等比缩到长边 ≤1600 + JPEG 降质到 ~700KB。
 * 拿不到 canvas 或仍超硬上限 → 返回 ''（丢图不阻断录单；只影响签证台取图）。
 */
export async function passportPhotoToDataUrl(file: File): Promise<string> {
