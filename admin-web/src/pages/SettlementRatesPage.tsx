 ...
 *   GET    /settlement-rates?from&to&nights   网格查询
 *   PUT    /settlement-rates/batch            批量 upsert（整批保存 / Excel 粘贴块）
 *   DELETE /settlement-rates/:id              删除一格
 *
 * 口径：选「月份 + 晚数」→ 网格（行 = 该月每天，列 = 四档）。格子直接编辑、整批保存；
 * 支持从 Excel 复制块状粘贴（tab/换行解析）；代理下套餐单时按去程出发日期在此表自动取每人价。
 */
...
function cellKey(date: string, tier: SettlementTier): string {
  return `${date}__${tier}`;
}
...
export function SettlementRatesPage() {
  ...
  const rateByKey = useMemo(() => {
    ...
    for (const r of rates) map.set(cellKey(r.departDate, r.tier), r);
    return map;
  }, [rates]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.listSettlementRates(token, { from, to, nights });
      setRates(res.rates);
      const next = new Map<string, string>();
      for (const r of res.rates) next.set(cellKey(r.departDate, r.tier), String(r.pricePerPersonCny));
      setDraft(next);
    } catch (e: unknown) {
  ...
  }, [token, month, nights, nonce]);

  function setCell(date: string, tier: SettlementTier, value: string) {
    setDraft((prev) => {
      const next = new Map(prev);
      next.set(cellKey(date, tier), value);
      return next;
    });
  }

  /**
   * Excel 块状粘贴：从命中的格子（startDay × startTier）向右下铺开填充。
   * 行按换行、列按 tab 拆分；命中多格时接管默认粘贴（preventDefault），单值粘贴走浏览器默认。
   */
    ...
    setDraft((prev) => {
      ...
      rows.forEach((row, r) => {
        row.split('\t').forEach((raw, c) => {
          ...
          const tierIdx = startTierIdx + c;
          if (dayIdx >= days.length || tierIdx >= TIERS.length) return;
          next.set(cellKey(days[dayIdx], TIERS[tierIdx]), normalizePasteCell(raw));
        });
      });
      return next;
    });
      ...
      for (const date of days) {
        for (const tier of TIERS) {
          ...
          if (!Number.isFinite(v) || !Number.isInteger(v) || v < 0) {
            throw new Error(`${date} ${TIER_LABELS[tier]} 的价格「${trimmed}」不是有效整数`);
          }
    ...
    <div className="space-y-6">
      ...
      <section className="card space-y-4">
        ...
        <p className="text-[11px] text-ink-muted">
          提示：可从 Excel 复制一块区域，选中起始格后直接粘贴（Ctrl/⌘+V）批量填充；清空格子并保存即删除该价。
        </p>
                        ...
                        return (
                          <td key={tier} className="px-1 py-1">
                            <input
                              ...
                              onChange={(e) => setCell(date, tier, e.target.value)}
                              onPaste={(e) => handlePaste(e, dayIdx, tierIdx)}
                            />
                          </td>
                        );
                      })}
